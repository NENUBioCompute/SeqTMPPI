...
myprotein = Protein()
class ComposeData:
    def save(self,dirout,flist, ratios,limit,labels=None,sep='\t',filename = 'all.txt',groupcount=1,repeate=True):
        """
        ...
        """
        for idx,elem in enumerate(self.doCompose(flist, ratios, limit,labels=labels,groupcount=groupcount,repeate=repeate)):
            data = pd.concat(elem)
        ...
        # data = data.sample(frac=1)
        # quick_save(data,fout,sep=sep)
    def doCompose(self,flist,ratios,limit,labels=None,groupcount=1,repeate=True):
        if repeate:return self.compose(flist,ratios,limit,labels=labels,groupcount=groupcount)
        else:return self.compose_norepeat(flist,ratios,limit,labels=labels,groupcount=groupcount)
    '''
    2049:7177
    1:2 可以得到two group of data
    正样本随机抽取（各组之间可能重复），负样本不重复，用于正样本较少，负样本较多的情况
    '''
    def compose(self,flist,ratios,limit,labels=None,groupcount=1):
        ratios = calcu_ratio(ratios)
        src_datalist, _min,_nums = self.load(flist)
        np_ratios = np.array(ratios[1:])
        np_nums = np.array(_nums[1:])

        max_group = 1 if limit == 0 else int(min(np_nums/(limit*np_ratios)))
        group_datasets = []
        for group in range(max_group):
            ...
            for idx,each in enumerate(src_datalist):
                _num = min(_nums) if limit ==0 else min(int(limit * ratios[idx]), _nums[idx])
                if idx ==0: # positive
                    data = each.sample(_num)
                    print('%d group: load %d data from %s'%(group,_num,flist[idx]))
                else:
                    # data = each.sample(_num)
                    data = each[group * _num:(group+1) * _num]
                    print('%d group: load %d [%d:%d] data from %s'%(group,_num,group * _num,(group+1) * _num,flist[idx]))
                if labels != None:
...
class PairDealer:
    ...
    def part(self,fin,ratios,fouts):
        ...
        for idx,fout in enumerate(fouts):
            ...
            start = end
    def sample(self,fin,num,fout):
        data = pd.read_table(fin, header=None)
        data = data.sample(frac=1)[:num]
        data.to_csv(fout,header=None,index=None,sep='\t')
...
def concatPAN(fin_posi, fin_nega, fout_pair):
    '''
    ...
    concatPAN(fin_posi, fin_nega, fout_pair)
    '''
    df_posi = pd.read_csv(fin_posi, sep='\t', header=None)
    df_nega = pd.read_csv(fin_nega, sep='\t', header=None)
    df_posi[2] = 1
    df_nega[2] = 0
    df_both = pd.concat([df_posi, df_nega])
    df_both.to_csv(fout_pair, header=None, index=None, sep='\t')
